...
function WalkerConfirmationPage() {
  ...
  const handleConfirm = async (bookingId, action) => {
    try {
      ...
      // If accepted, show alert asking if want to start tracking
      if (action === "accepted") {
        ...
        showAlert({
          ...
          message: `Reservasi ditolak! Pet Owner akan segera diinfokan.`,
          type: "success",
          confirmText: "OK",
        });
      ...
      const errorMessage =
        ...
        "Gagal mengupdate booking. Silakan coba lagi.";
      showAlert({
        title: "Error",
  ...
  const handleStartTracking = () => {
    navigate("/walker-tracking", {
      state: {
        bookingId: trackingAlert.bookingId,
        ownerName:
          trackingAlert.bookingInfo?.owner?.name ||
          trackingAlert.bookingInfo?.owner_name ||
          "Pet Owner",
      },
    });
    setTrackingAlert({ isOpen: false, bookingId: null, bookingInfo: null });
  };

  const handleSkipTracking = () => {
    showAlert({
      title: "Reservation Accepted",
      message: "Booking diterima! Pet Owner akan segera diinfokan.",
      type: "success",
      confirmText: "OK",
    });
    setTrackingAlert({ isOpen: false, bookingId: null, bookingInfo: null });
  ...
  if (error) {
    return (
      <div className="walker-confirm-page-container">
        <Header />
        <div className="status-page-main" style={{ marginTop: "50px" }}>
          <h2 className="status-message" style={{ color: "#ff6b6b" }}>
            {error}
          </h2>
          <button
  ...
  return (
    <div className="walker-confirm-page-container">
      ...
      <div className="walker-confirm-page-main">
        ...
        <div className="confirm-welcome-text">
          <h2 className="confirm-heading">
            Look, there's {bookings.length} new <br />
            reservation{bookings.length > 1 ? "s" : ""} for you 👑
          </h2>
          <p className="confirm-subtext">
            Go with the best choice for yourself
          </p>
        </div>
          ...
          {bookings.map((booking) => (
            <div key={booking.id} className="reservation-details-box">
              ...
              <div className="reservation-details-row">
                <span>Pet Owner</span>
                <span>
                  : {booking.owner?.name || booking.owner_name || "Unknown"}
                </span>
              </div>
              <div className="reservation-details-row">
                <span>Phone</span>
              ...
              <div className="reservation-details-row">
                <span>Address</span>
                <span>
                  : {booking.owner?.address || booking.owner_address || "-"}
                </span>
              </div>
              <div className="reservation-details-row">
                <span>Date & Time</span>
      ...
      <AlertModal
        isOpen={trackingAlert.isOpen}
        title="Start Tracking"
        message="Booking diterima! Apakah Anda ingin mulai tracking sekarang?"
        type="confirm"
        confirmText="Yes, Start Tracking"
